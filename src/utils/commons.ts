...
export const separator = (num: number, delim = '='): string => {
    return new Array(num).join(delim)
}
...
export const hasProperty = (value: any, prop: PropertyKey): boolean => {
    const proto = value.__proto__ || value.constructor.prototype

    //return (prop in obj) && (!(prop in proto) || proto[prop] !== obj[prop]);
    return prop in value || prop in proto || proto[prop] === value[prop]
}

export const pluck = <T, K extends keyof T>(value: T, propertyNames: K[]): T[K][] => {
    return propertyNames.map(n => value[n])
}
